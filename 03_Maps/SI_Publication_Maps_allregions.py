...
# 1. Pfade anpassen
# =========================================
excel_path = r"/Users/philina/Desktop/TUM/4. Semester/Masterarbeit/01_MA Vulnerability Framework/05_Scripts/02_Master_Framework/output/Master_Framework.xlsx"
...
output_dir = r"/Users/philina/Desktop/TUM/4. Semester/Masterarbeit/01_MA Vulnerability Framework/06_Mapping/00_Publication/02_Review_all_Countries"
...
df = pd.read_excel(excel_path, sheet_name="Index Data")
...
indicator_cmaps = {
    "Index Transition Exposure Review": make_fresh_hex_cmap("#0A2A58", "EE_cmap"),
    "Adaptive Capacity Review": make_fresh_hex_cmap("#0E5B2C", "AC_cmap"),
    "Index Sensitivity Review": make_fresh_hex_cmap("#510A94", "SS_cmap"),
}
...
for col in index_cols_to_map:
    ...
    cmap = indicator_cmaps.get(col) or indicator_cmaps.get(base_name_clean)
    if cmap is None:
    ...
    not_jtf_patch = mpatches.Patch(
        facecolor="lightgrey",                  
        edgecolor="white",
        label="no data/non EU"
    )

    if col == "Adaptive Capacity Review":
        ax.legend(
            handles=[not_jtf_patch],
            loc="upper right",
